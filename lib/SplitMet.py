...
from lib.VisualizationReachSet import *
from lib.ComputeU import *
from lib.SamplingMet import *
from lib.Gridding import *

import lib.Profiling
...
class Split:
    ...
    def printReachableSetAll(self,s1,s2,n):
        ...
        while (t<=self.T):
            ...
            if t%intervalPlot==0:
                (X1,Y1)=Visualization(s1,s2,RS).getPlotsLineFine()
                (X2,Y2)=Visualization(s1,s2,ORS_compact_interval).getPlotsLineFine()
                (X3,Y3)=Visualization(s1,s2,ORS).getPlotsLineFine()
                lst=Sampling.getPlotsLineFine(s1,s2,SRS)
                lPlots=[lst,[X1,Y1,X2,Y2,X3,Y3]]
                name=n+"_"+str(t)
    ...
    def getReachableSetRed(self):
        ...
        #lPlots=[[(X5,Y5)],[X1,Y1,X2,Y2,X3,Y3,X4,Y4]]
        #images.append(Visualization.getPlotAll(s1,s2,lPlots,name+"_0"))
        ORS_List=[copy.copy(ORS_compact_interval)]
        RS_List=[copy.copy(RS)]
        ...
        while (t<=self.T):
            ...
            ORS_List.append(copy.copy(ORS_compact_interval))
        ...
        t=t-1
        #images[0].save("GIFs/"+n+'.gif',save_all=True, append_images=images[1:], optimize=False, duration=250, loop=0)
        print("----Timining Details for----")
        print("With Reduction: ",time_taken_interval)
        #print("With Reduction (Zono): ",time_taken_reduction_zono)
        #print("With Reduction (Interval): ",time_taken_reduction_interval)
        #print("Total Time: ",time.time()-start_time_total)

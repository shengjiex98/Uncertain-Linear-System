...
class Visualization:
    ...
    def getPlots(self):
        ...
        for i in range(len(P3)):
            name="Pred"+str(i)
            predVars.append(model3.addVar(-GRB.INFINITY,GRB.INFINITY,name=name,vtype='C'))
        #-----------------------

        # Axes Variables
        X=model3.addVar(-GRB.INFINITY,GRB.INFINITY,name="X",vtype='C')
        Y=model3.addVar(-GRB.INFINITY,GRB.INFINITY,name="Y",vtype='C')
        #-------------------------

        # Create the Star Constraints
        objX=0
        for i in range(V3.shape[1]):
            objX=objX+(predVars[i]*V3[self.theta1][i])
        objX=C3[self.theta1]+objX

        objY=0
        for i in range(V3.shape[1]):
            objY=objY+(predVars[i]*V3[self.theta2][i])
        objY=C3[self.theta2]+objY

        model3.addConstr(X==objX,"X Axis")
        model3.addConstr(Y==objY,"Y Axis")
        #-----------------------------------
        ...
        for i in range(len(P3)):
            ...
            name="Pred C"+str(i)
            if a==b:
                model3.addConstr(predVars[i]==a,name)
            else:
                model3.addConstr(predVars[i]>=min(a,b),name+".1")
                model3.addConstr(predVars[i]<=max(a,b),name+".2")
        #-----------------------------------
        ...
        for an in range(90):
            if an==0:
                model3.addConstr(X==0,"Angle")
            else:
                m=math.tan(math.radians(an))
                model3.addConstr(Y==m*X,"Angle")
            try:
                model3.optimize()
                #model2.write("dump.lp")
                #exit(0)
                ...
                if status==GRB.Status.UNBOUNDED:
                    ...
                    if status == GRB.Status.INF_OR_UNBD or \
                       ...
                       status == GRB.Status.UNBOUNDED:
                        ...
                        xVal=model3.getVarByName("X").x
                        yVal=model3.getVarByName("Y").x
                        X_list3.append(xVal)
                        Y_list3.append(yVal)
            except:
                semiDefFlag=True
            ...
            semiDefFlag=False
            model3.remove(model3.getConstrByName("Angle"))
        #-----------------------------
        ...
        return (X_list,Y_list,X_list2,Y_list2,X_list3,Y_list3)
    ...
    def addStars2(self):
        '''
        Given two stars self.st1 and self.st3, this functions performs
        minkowski sum of the two stars.
        '''
    ...
    def displayPlot(self):
        (X1,Y1,X2,Y2,X3,Y3)=self.getPlots()
        plt.axes()
        ...
        plt.plot(X2,Y2,'r+',label="Perturbed")
        plt.plot(X3,Y3,'kx',label="Perturbed2")
...
if False:
    ...
    P3=[(-0.2,0.2), (-0.7,0.7)]
    rs3=(C3,V3,P3)
    v=Visualization(0,1,rs,rs2,rs3)
    v.displayPlot()
